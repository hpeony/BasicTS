...
import torch
# ==================== 新增导入 ====================
from .layers import TemporalExternalAttnLayer, SpatialExternalAttnLayer
    ...
    def __init__(
        ...
        if self.use_temporal_external_attn:
            self.attn_layers_t = nn.ModuleList(
                [
                    TemporalExternalAttnLayer(self.model_dim, self.in_steps, self.num_nodes, feed_forward_dim, dropout,
                                              external_memory_S)
        ...
        if self.use_spatial_external_attn:
            self.attn_layers_s = nn.ModuleList(
                [
                    SpatialExternalAttnLayer(self.model_dim, self.in_steps, self.num_nodes, feed_forward_dim, dropout,
                                             external_memory_S)
                    for _ in range(num_layers)
                ]
            )
